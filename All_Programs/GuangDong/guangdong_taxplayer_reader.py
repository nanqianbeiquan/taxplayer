...
class GuangDongTaxplayerReader(TaxplayerReader):
    ...
    def word_to_html(self):
        sql = "SELECT * from taxplayer_filename where (title like '%欠税%' or title like '%缴%' or" \
              " title like '%非正常户%') and filename like '%.doc%' and province = '" + self.province.encode('utf8') + \
    ...
    def abnormal_excel_reader(self):
            ...
            try:
                ...
                for m in range(count):
                    try:
                        ...
                        print num_info + 1, filepath, u'读取非正常户excel字段信息错误（get_excel_abnormal_field_info)'
                        print 'e: ',e
                ...
                if '<html' in str(e[0]) or '<!DOCT' in str(e[0]):
                    try:
                        ...
                        print num_info + 1, filepath, u'将非正常户excel文件作为html读取不成功'
                        print 'e:',e
                        self.log(str(num_info + 1) + ',' + 'filepath  ' + filepath)
                        self.log(u'将非正常户excel文件作为html读取不成功,e:  ' + str(e))
                else:
                    print num_info + 1,filepath ,u'非正常户excel读取失败'
                    print 'e: ',e
        ...
        self.log(self.province + u'一共插入' + str(num_sql_all) + u'条')
        self.log(self.province + u'一共重复' + str(num_repetition_all) + u'条')
        self.log(self.province + u'一共失败' + str(num_fail_all) + u'条')
        print self.province + u'一共插入' + str(num_sql_all) + u'条'
        print self.province + u'一共重复' + str(num_repetition_all) + u'条'
        print self.province + u'一共失败' + str(num_fail_all) + u'条'
            ...
            try:
                ...
                for m in range(count):
                    try:
                        ...
                        if str(e[0]) == 'need more than 0 values to unpack':
                            ...
                            print num_info + 1, filepath, u'读取欠税公告excel字段信息错误（get_excel_qsgg_field_info)'
                            print 'e: ',e
                            self.log(str(num_info+1) + 'filepath  ' + filepath)
                        ...
                        if match_fields:
                            ...
                            if 'nsrsbh' not in keys or 'nsrmc' not in keys:
                                print 'not nsrsbh or nsrmc ', num_info + 1, info[num_info][4]
                            if 'qsje' not in keys and not cqsje_condition:
                                print 'nnnn', num_info + 1, info[num_info][4]
                                continue
                            print num_info + 1, match_fields, wan
                            # continue
                            start_idx = self.get_excel_start_idx(table, rows)
                ...
                if '<html' in str(e[0]) or '<!DOCT' in str(e[0]):
                    try:
                        ...
                        print num_info + 1, filepath, u'将欠税公告excel文件作为html读取不成功'
                        print 'e:',e
                        self.log(str(num_info + 1) + ',' + 'filepath  ' + filepath)
